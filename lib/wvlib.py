...
class OneHotWVData(WVData):
    ...
    def __init__(self, word_idx):
        ...
        logging.info('word2vec load: filling in 0s for word counts')
...
class SdvData(WVData):
    def __init__(self, data):
        ...
        logging.info('sdv load: filling in 0s for word counts')
...
#         return = ''.join(iter(lambda: f.read(1), ' '))
        ...
        while True:
            ...
            if not c:
                raise FormatError("premature end of file")
            wchars.append(c)
        if wchars and isinstance(wchars[0], bytes):
            return b''.join(wchars).decode('utf-8')
        else:
            return ''.join(wchars)
